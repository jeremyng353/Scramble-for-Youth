...
## Getting Started
Simply pull the repository and run 'node web_app/server.js' in bash. If playing without hardware components, use the web-version branch via 'git switch web-version'. 
...
## Hardware Components
Scramble for Youth is intended to be played with several hardware components hooked up to a DE1-SoC, such as using an accelerometer to determine the exact range of an attack, interfacing through a VGA monitor to play as the game master, and having the AI logic in hardware. However, it is possible to play with just four players in the web-version branch.